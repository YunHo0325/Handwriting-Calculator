'''정보 출처
원본 : https://github.com/IBM/tensorflow-hangul-recognition
TFRecord : https://bcho.tistory.com/1190
freeze_graph.freeze_graph : https://medium.com/@prasadpal107/saving-freezing-optimizing-for-inference-restoring-of-tensorflow-models-b4146deb21b5
GraphDef : https://developers.google.com/protocol-buffers/docs/pythontutorial
optimize_for_inference : https://meisteruser.net/devflow/2294
initial : https://tensorflowkorea.gitbooks.io/tensorflow-kr/content/g3doc/api_docs/python/constant_op.html
'''


'''TFRecord 를 사용하는 이유
1. 학습을 위한 데이터와 그 데이터의 의미를 담은 label 정보로 구성돼 있다. ... 코드 구현시 효율 증가
2. 이미지 파일을 학습할 경우, 이미지를 매번 인코딩/디코딩 작업을 해줄 필요가 없다. ... 학습 속도 개선
3. 이미지 파일을 TFRecord 파일로 생성할때 사이즈가 작아진다. ... 파일을 다루기 편함
'''

'''코드의 구성
1. TFRecord 를 읽기 위한 파이프라인 생성과, 일괄처리 데이타를 무작위로 선정
2. CNN 으로 학습
3. Android 에서 사용 가능하도록 출력
'''
...
# Default paths.
DEFAULT_LABEL_FILE = os.path.join(SCRIPT_PATH, './labels/label.txt')
...
# Step : BatchSize 만큼 학습하는 횟수
BATCH_SIZE = 100
# BatchSize : 한번에 네트워크에 넘겨주는 데이터의 수 ... 메모리공간 효율적으로 사용
...
def get_image(files, num_classes):
    ...
    '''Queue Runner
    여러개의 파일, 큰 용량의 파일을 학습하기 위해
    파일을 나누어서 큐에 저장하는 방식으로 학습을 진행한다.
    
    아래는 큐러너 사용을 위한 코드로 구성되어 있다.
    '''
    ...
    # 2. 파일 형식에 따라 Reader 를 선택한다. (csv 파일의 경우 tf.TextLineReader() 사용)

    # Read the full set of features for a single example.
    key, example = reader.read(file_queue)
    # 3. 선택한 reader 로 읽기를 실행한다.
    ...
    '''TFRecord 읽기
    TFRecord 는 Feature 라는 배열에 딕셔너리 형태('사과':2500, '바나나':1400)로 저장한다.
    
    작성은             tf.train.Example(),         tf.train.Features() 로 하고
    해독(Parsing)은    tf.parse_single_example(),  tf.FixedLenFeature() 로 한다.
    '''
    ...
    # 1. 필요한 각 Feature 를 데이터 타입에 맞게 Parsing 한다.
...
def export_model(model_output_dir, input_node_names, output_node_name):
    ...
    ) : 학습이 끝난 변수와 그래프의 모든 값을 단일 파일로 저장하여 보관할 필요가 있을 때 사용한다.
    (함수 하나에 매개변수가 무려 10개나 들어가 있는 무시무시한 1줄짜리 코드;)
    '''

    input_graph_def = tf.GraphDef()
    # GraphDef() : GraphDef 클래스의 빈 객체를 생성 ... 우리가 가진 데이터를 채울 객채
    with tf.gfile.Open(frozen_graph_file, "rb") as f:
    # 파일 핸들 취득, 위에서 저장한 frozen_graph_file 사용
        input_graph_def.ParseFromString(f.read())
        # ParesFromString() : 바이너리 파일을 호출

    output_graph_def = optimize_for_inference_lib.optimize_for_inference(
            input_graph_def, input_node_names, [output_node_name],
            tf.float32.as_datatype_enum)
    # optimizer_for_inference : 그래프 파일(.pd)에 있는 모바일에서 지원하지 않는 operation 들을 제거
...
def weight_variable(shape):
    ...
    # tf.truncated_normal() : 절단정규분포로부터의 난수값을 반환
...
def main(label_file, tfrecords_dir, model_output_dir, num_train_steps):
    ...
    모델
        ...
        CONV1 ... Convolution(f:5*5*1, s:1), ReLU, Max-Pool(f:2*2*1, s:2)
        CONV2 ... Convolution(f:5*5*32, s:1), ReLU, Max-Pool(f:2*2*1, s:2)
        CONV3 ... Convolution(f:3*3*64, s:1) , ReLU, Max-Pool(f:2*2*1, s:2)
        FC1 ... weight[8*8*128, 1024], ReLU, Dropout : 50%
        FC2 ... weight[1024, label]
    '''
    ...
    # % : 문자열 포매팅 ... 왼쪽 문자열의 %d, %f, %s 등등 해당하는 곳에 오른쪽 문자열 대입
    train_data_files = tf.gfile.Glob(tf_record_pattern)
    # tf.gfile.Glob() : 지정된 패턴과 일치하는 파일 목록 반환
    ...
    with tf.Session() as sess:
        ...
        # tf_record_iterator() : TFRecord 파일에서 레코드 반복 읽기
        ...
        test_step = int(sample_count/BATCH_SIZE) or 1
        ...
        accuracy_percent = total_correct_preds/(test_step*BATCH_SIZE)
